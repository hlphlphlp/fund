...
def smart_sorting(funds_lst, mode='ranking'):
    ...
    for i in range(len(sort_by_1_week)):
        if mode == 'ranking':
            score_dic[sort_by_1_week[i]['code_id']] = [i*0.6]
        elif mode == 'rise':
    ...
    for i in range(len(sort_by_1_month)):
        if mode == 'ranking':
            score_dic[sort_by_1_month[i]['code_id']].append(i*0.9)
        elif mode == 'rise':
            score_dic[sort_by_1_week[i]['code_id']].append(sort_by_1_week[i]['one_month'])

    sort_by_3_month = sorted(funds_lst, key=operator.itemgetter('three_month'))
    for i in range(len(sort_by_3_month)):
        if mode == 'ranking':
            score_dic[sort_by_1_month[i]['code_id']].append(i)
        elif mode == 'rise':
            score_dic[sort_by_1_week[i]['code_id']].append(sort_by_1_week[i]['three_month'])
    ...
    for i in range(len(sort_by_6_month)):
        if mode == 'ranking':
            ...
            score_dic[sort_by_1_week[i]['code_id']].append(sort_by_1_week[i]['six_month'])

    sort_by_1_year = sorted(funds_lst, key=operator.itemgetter('one_year'))
    for i in range(len(sort_by_1_year)):
        if mode == 'ranking':
            score_dic[sort_by_1_month[i]['code_id']].append(i*1.1)
        elif mode == 'rise':
            score_dic[sort_by_1_week[i]['code_id']].append(sort_by_1_week[i]['one_year'])

    sort_by_3_year = sorted(funds_lst, key=operator.itemgetter('three_year'))
    for i in range(len(sort_by_3_year)):
        if mode == 'ranking':
            score_dic[sort_by_1_month[i]['code_id']].append(i*1.3)
        elif mode == 'rise':
            score_dic[sort_by_1_week[i]['code_id']].append(sort_by_1_week[i]['three_year'])
...
def sorting_funds():
    ...
    # smart排序
    smart_lst = []
    score_result_list = smart_sorting(result_funds_lst, mode='rise')
